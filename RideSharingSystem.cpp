...
private:
    ...
    std::unordered_set<int> CancelledRiders;

public:
    void addRider(int riderId) {
        if (CancelledRiders.find(riderId) != CancelledRiders.end())
        {
            CancelledRiders.erase(riderId);
        }
    ...
    std::vector<int> matchDriverWithRider() {
        while (!Riders.empty() &&
               CancelledRiders.count(Riders.front())) {
...
/**
 * Test the RideSharingSystem with Example 1
 */
int main() {
    RideSharingSystem obj;
    obj.addRider(3); // rider 3 joins the queue
    obj.addDriver(2); // driver 2 joins the queue
    obj.addRider(1); // rider 1 joins the queue
    auto res = obj.matchDriverWithRider(); // returns [2, 3]
    std::cout << "[" << res[0] << ", " << res[1] << "]" << std::endl;
    obj.addDriver(5); // driver 5 becomes available
    obj.cancelRider(3); // rider 3 is already matched, cancel has no effect
    res = obj.matchDriverWithRider(); // returns [5, 1]
    std::cout << "[" << res[0] << ", " << res[1] << "]" << std::endl;
    res = obj.matchDriverWithRider(); // returns [-1, -1]
    std::cout << "[" << res[0] << ", " << res[1] << "]" << std::endl;
    return 0;
}
